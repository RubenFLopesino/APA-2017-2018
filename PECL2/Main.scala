...
object Main extends App {
  ...
  //En este método se elige la fila, y se comprueba que está dentro de unos rangos
  def elegirFila(filas:Int): Int = {
    val fila = Integer.parseInt(Console.in.readLine())
		if(fila < 1 || fila > 7){
			//Si se sale del rango que nos interesa, llamamos a controlFilas
		  controlFilas(filas)
		}
		else{
			//Si no, devolvemos la fila elegida
		  fila
		}
  }
  
  def elegirColumna(columnas:Int): Int = {
    val columna = Integer.parseInt(Console.in.readLine())
		if(columna < 1 || columna > 7){
		  controlColumnas(columnas)
		}
		else{
		  columna
		}
  ...
  def controlFilas(filas:Int): Int ={
    println("La fila elegida no es válida, introduzca un valor entre 1 y " + filas)
    val eleccion =  Integer.parseInt(Console.in.readLine())
    if(eleccion < 1 || eleccion > filas){
      controlFilas(filas)
    }
    else{
      eleccion
  ...
  def controlColumnas(columnas:Int): Int ={
    println("La columna elegida no es válida, introduzca un valor entre 1 y " + columnas)
    val eleccion =  Integer.parseInt(Console.in.readLine())
    if(eleccion < 1 || eleccion > columnas){
      controlFilas(columnas)
    }
    else{
      eleccion
    }
  ...
  def imprimirTablero(columnas:Int, filas:Int, fila:Int, columna:Int, tablero:List[Int]): Unit = {
	...
	//Metodo para génerar el tablero con números aleatorios
	...
	def jugarToyBlast(tablero:List[Int], dificultad:Int, puntuacion:List[Int]): Unit ={
	  if(dificultad == 1){
	  	...
	  	if(devuelveBloque(puntuacion,0) >= 20 && devuelveBloque(puntuacion,1) >= 20 && devuelveBloque(puntuacion,2) >= 20 && devuelveBloque(puntuacion,3) >= 20){
		    ...
		    println("Introduzca el n de fila: ")
		    val fila = elegirFila(7)
		    println("Introduzca el n de columna: ")
		    val columna = elegirColumna(9)
	  	...
	  	if(devuelveBloque(puntuacion,0) >= 15 && devuelveBloque(puntuacion,1) >= 15 && devuelveBloque(puntuacion,2) >= 15 && devuelveBloque(puntuacion,3) >= 15 && devuelveBloque(puntuacion,4) >= 15){
		    ...
		    println("Introduzca el n de fila: ")
		    val fila = elegirFila(11)
		    println("Introduzca el n de columna: ")
		    val columna = elegirColumna(17)
		...
		if(devuelveBloque(puntuacion,0) >= 10 && devuelveBloque(puntuacion,1) >= 10 && devuelveBloque(puntuacion,2) >= 10 && devuelveBloque(puntuacion,3) >= 10 && devuelveBloque(puntuacion,4) >= 10 && devuelveBloque(puntuacion,5) >= 10){
		   println("Has ganado, ¡FELICIDADES!")
		    ...
		    println("Introduzca el n de fila: ")
		    val fila = elegirFila(15)
		    println("Introduzca el n de columna: ")
		    val columna = elegirColumna(27)
